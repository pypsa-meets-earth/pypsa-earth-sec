...
def extract_res(n):
    res_caps = pd.DataFrame(
        index=n.buses[n.buses.carrier == "AC"].index, columns=technologies
    )

    for tech in technologies:
        res_ind = n.generators[n.generators.carrier == tech].index
        res_caps_vals = n.generators.loc[res_ind, "p_nom_opt"]
        res_caps_vals.index = res_caps_vals.index.to_series().apply(
            lambda x: x.split(" ")[0]
        )
    ...
    res_caps.to_csv(
        ...
        + n_path.split("/")[-1]
        .replace(".nc", ".csv")
        .replace("elec", "res_caps_elec")
        .replace("_" + str(ex_q_30), "_" + str(ex_q_50))
    )
...
def extract_electrolyzers(n):
    ...
    n.links.loc[elec_ind].p_nom_opt.to_csv(
        parent_dir
        + "/"
        + n_path.split("/")[-1]
        .replace(".nc", ".csv")
        .replace("elec", "electrolyzer_caps_elec")
        .replace("_" + str(ex_q_30), "_" + str(ex_q_50))
    )
...
def extract_pipelines(n):
    ...
    n.links.loc[ppl_ind].p_nom_opt.to_csv(
        parent_dir
        + "/"
        + n_path.split("/")[-1]
        .replace(".nc", ".csv")
        .replace("elec", "pipeline_caps_elec")
        .replace("_" + str(ex_q_30), "_" + str(ex_q_50))
    )
    ...
    for year in [2030]:
        for s in ["BS", "AP", "NZ"]:
            for export in ex_quantities:
                n_paths.append(
                    "results/{}/postnetworks/elec_s_{}_ec_lc1.0_Co2L_3H_{}_{}_{}_{}export.nc".format(
