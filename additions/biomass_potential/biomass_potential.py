...
biomass_potential_ukraine = pd.read_excel(
    fn,
    index_col=0,
    storage_options=storage_options,
    sheet_name="dm_BASE_2030_kton",
    header=0,
)

biomass_potential_ukraine = biomass_potential_ukraine.reset_index().rename(
    columns={"index": "ID_S2biom_DB_L3"}
)
...
biomass_potential_ukraine_transposed = (
    biomass_potential_ukraine.transpose().reset_index()
)  # .set_index('ID_S2biom_DB_L3')

# Set the first row as column headers
biomass_potential_ukraine_transposed.columns = (
    biomass_potential_ukraine_transposed.iloc[0]
)
biomass_potential_ukraine_transposed = biomass_potential_ukraine_transposed[
    1:
...
biomass_potential_ukraine_transposed[
    "ID_S2biom_DB_L3"
] = biomass_potential_ukraine_transposed["ID_S2biom_DB_L3"].astype("int64")
...
merged_df1 = pd.merge(
    biomass_potential_ukraine_transposed,
    types[["type_id", "categorie"]],
    left_on="ID_S2biom_DB_L3",
    right_on="type_id",
)  # ['type_id', 'categorie', 'short_name'] --> if we want to have multiindex later
merged_df1
...
merged_df = pd.merge(
    merged_df1_T,
    regions[["ID_S2biom_DB_L3", "name"]],
    left_on="categorie",
    right_on="ID_S2biom_DB_L3",
)
merged_df
...
solid_biomass_potential = (
    ...
    .groupby(["name"])[
        "1111", "1112", "1113", "1114", "1211", "1212", "1213", "1214", "1221", "1222"
    ]
    .sum()
    .sum(axis=1)
)
...
# %%
biogas_potential = df[
    ["name", "2112", "2113", "2114", "2115", "2116", "5111", "5112"]
].set_index("name")
...
not_included_potential = (
    df.set_index("name").sum(axis=1)
    - biogas_potential["biogas_potential"]
    - solid_biomass_potential["solid_biomass_pot"]
)
not_included_potential
