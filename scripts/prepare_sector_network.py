...
def prepare_transport_data(n):
    ...
    nodal_energy_totals = energy_totals.loc[n.buses.country.dropna()].fillna(0.0)
    ...
    nodal_energy_totals = nodal_energy_totals.multiply(
        0.25, axis=0
    )  # TODO change 0.25 by pop_layout.fraction
    ...
    nodal_transport_data = transport_data.loc[n.buses.country.dropna()].fillna(0.0)
    nodal_transport_data.index = nodes
    # TODO pop_layout["fraction"] instead of 0.25
    nodal_transport_data["number cars"] = 0.25 * nodal_transport_data["number cars"]
    ...
    dsm_profile = generate_periodic_profiles(
        dt_index=n.snapshots.tz_localize("UTC"), nodes=nodes, weekly_profile=dsm_week
    )
