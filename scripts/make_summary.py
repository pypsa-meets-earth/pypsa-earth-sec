...
def calculate_nodal_cfs(n, label, nodal_cfs):
    ...
    for c in n.iterate_components(
        ...
        capacities_c = c.df.groupby(["location",
                                     "carrier"])[opt_name.get(c.name, "p") +
                                                 "_nom_opt"].sum()
        ...
        index = pd.MultiIndex.from_tuples([(c.list_name, ) + t
                                           for t in cf_c.index.to_list()])
...
def calculate_cfs(n, label, cfs):
    ...
    for c in n.iterate_components(n.branch_components
                                  ...
                                  ^ {"Load", "StorageUnit"}):
        capacities_c = (c.df[opt_name.get(c.name, "p") + "_nom_opt"].groupby(
            c.df.carrier).sum())
...
def calculate_nodal_costs(n, label, nodal_costs):
    ...
    for c in n.iterate_components(n.branch_components
                                  | n.controllable_one_port_components
                                  ^ {"Load"}):
        c.df["capital_costs"] = (c.df.capital_cost *
                                 c.df[opt_name.get(c.name, "p") + "_nom_opt"])
        capital_costs = c.df.groupby(["location",
                                      "carrier"])["capital_costs"].sum()
        index = pd.MultiIndex.from_tuples([
            (c.list_name, "capital") + t
            for t in capital_costs.index.to_list()
        ])
        ...
        if c.name == "Store":
            items = c.df.index[(c.df.carrier == "co2 stored")
                               & (c.df.marginal_cost <= -100.0)]
            c.df.loc[items, "marginal_cost"] = -20.0

        c.df["marginal_costs"] = p * c.df.marginal_cost
        marginal_costs = c.df.groupby(["location",
                                       "carrier"])["marginal_costs"].sum()
        index = pd.MultiIndex.from_tuples([
            (c.list_name, "marginal") + t
            for t in marginal_costs.index.to_list()
        ])
        nodal_costs = nodal_costs.reindex(index.union(nodal_costs.index))
...
def calculate_costs(n, label, costs):
    ...
    for c in n.iterate_components(n.branch_components
        ...
        capital_costs_grouped = pd.concat([capital_costs_grouped],
                                          keys=["capital"])
        capital_costs_grouped = pd.concat([capital_costs_grouped],
                                          keys=[c.list_name])
        ...
        marginal_costs_grouped = pd.concat([marginal_costs_grouped],
                                           keys=["marginal"])
        marginal_costs_grouped = pd.concat([marginal_costs_grouped],
                                           keys=[c.list_name])
...
def calculate_cumulative_cost():
    ...
    for r in cumulative_cost.columns:
        cumulative_cost[r] = [
            df["costs"].sum()[index] /
            ((1 + r)**(index[-1] - planning_horizons[0]))
            for index in cumulative_cost.index
        ]

    # integrate cost throughout the transition path
    for r in cumulative_cost.columns:
        for cluster in cumulative_cost.index.get_level_values(
                level=0).unique():
            for lv in cumulative_cost.index.get_level_values(level=1).unique():
                for sector_opts in cumulative_cost.index.get_level_values(
                        level=2).unique():
                    cumulative_cost.loc[(
                        cluster, lv, sector_opts,
                        "cumulative cost"), r] = np.trapz(
                            cumulative_cost.loc[idx[cluster, lv, sector_opts,
                                                    planning_horizons],
                                                r].values,
                            x=planning_horizons,
                        )
...
def calculate_nodal_capacities(n, label, nodal_capacities):
    ...
    for c in n.iterate_components(n.branch_components
                                  | n.controllable_one_port_components
                                  ^ {"Load"}):
        nodal_capacities_c = c.df.groupby(
            ["location",
             "carrier"])[opt_name.get(c.name, "p") + "_nom_opt"].sum()
        index = pd.MultiIndex.from_tuples([
...
def calculate_capacities(n, label, capacities):
    ...
    for c in n.iterate_components(n.branch_components
                                  | n.controllable_one_port_components
                                  ^ {"Load"}):
        capacities_grouped = (c.df[opt_name.get(c.name, "p") +
                                   "_nom_opt"].groupby(c.df.carrier).sum())
        capacities_grouped = pd.concat([capacities_grouped],
                                       keys=[c.list_name])
    ...
    for c in n.iterate_components(n.one_port_components | n.branch_components):
        ...
        if c.name in n.one_port_components:
            ...
            for port in [col[3:] for col in c.df.columns if col[:3] == "bus"]:
                ...
                totals.loc[no_bus] = n.component_attrs[c.name].loc["p" + port,
                                                                   "default"]
...
def calculate_supply_energy(n, label, supply_energy):
    ...
    for i in bus_carriers:
        ...
        for c in n.iterate_components(n.one_port_components):
            ...
            s = (c.pnl.p[items].multiply(n.snapshot_weightings.generators,
                                         axis=0).sum().multiply(
                                             c.df.loc[items, "sign"]).groupby(
                                                 c.df.loc[items,
                                                          "carrier"]).sum())
            s = pd.concat([s], keys=[c.list_name])
            s = pd.concat([s], keys=[i])
        ...
        for c in n.iterate_components(n.branch_components):
            ...
            for end in [col[3:] for col in c.df.columns if col[:3] == "bus"]:
                ...
                s = (-1) * c.pnl["p" + end][items].multiply(
                    n.snapshot_weightings.generators, axis=0).sum().groupby(
                        c.df.loc[items, "carrier"]).sum()
                s.index = s.index + end
                s = pd.concat([s], keys=[c.list_name])
                s = pd.concat([s], keys=[i])
...
def calculate_market_values(n, label, market_values):
    ...
    for tech in techs:
        ...
        dispatch = (n.generators_t.p[gens].groupby(n.generators.loc[gens,
                                                                    "bus"],
                                                   axis=1).sum().reindex(
                                                       columns=buses,
                                                       fill_value=0.0))
        ...
        market_values.at[tech,
                         label] = revenue.sum().sum() / dispatch.sum().sum()
    ...
    for i in ["0", "1"]:
        ...
        for tech in techs:
            ...
            dispatch = (n.links_t["p" + i][links].groupby(
                n.links.loc[links,
                            "bus" + i], axis=1).sum().reindex(columns=buses,
                                                              fill_value=0.0))
...
def calculate_price_statistics(n, label, price_statistics):
    ...
    price_statistics.at["zero_hours",
                        label] = df.sum().sum() / (df.shape[0] * df.shape[1])
...
if __name__ == "__main__":
    ...
    networks_dict = {
        ...
        for cluster in snakemake.config["scenario"][
            "clusters"]  # for opt in snakemake.config['scenario']['opts'] \
        # for sector_opt in snakemake.config['scenario']['sector_opts'] \
        # for lv in snakemake.config['scenario']['lv'] \
        for planning_horizon in snakemake.config["scenario"]
        ["planning_horizons"]
    }
    ...
    if snakemake.config["foresight"] == "myopic":
        ...
        cumulative_cost.to_csv(snakemake.config["summary_dir"] + "/" +
